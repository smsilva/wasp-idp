...
Recomendação: **ALB em `connectivity/`, como o plano diz**, aceitando que ele desce junto com o T1. O ingress público de uma célula de PoC não precisa sobreviver à noite, e o alternativo — ALB numa raiz permanente própria — cria uma quinta camada por um recurso só. Registrar a consequência: **derrubar o T1 derruba o ingress público**, e o `curl` de aceite do `3.2` só funciona com a camada 03 de pé.
...
6. **`hub_public_subnet_ids`**, para o ALB consumir. Alternativa consistente com o resto do repo: `data "aws_subnets"` filtrando por `tag:Name = <nome-do-hub>-public-*`. Preferir o output: é a mesma raiz que já expõe as privadas, e um filtro por padrão de nome quebra em silêncio se o nome mudar.

### Em `control-plane/` — por cluster, via provider `aws.network`

7. **Certificado wildcard no ACM**: `*.<var.name>.nonprod.<domínio>`, validação por **DNS**, com os registros de validação na subzona da camada 02 e um `aws_acm_certificate_validation` para o apply esperar. Wildcard cobre **um nível só**: `*.nonprod.<dom>` não cobre `app.<id>.nonprod.<dom>`, e `*.*.` não existe.
8. **`aws_lb_listener_certificate`** anexando o certificado ao listener `:443` compartilhado (é o SNI que permite N clusters num listener só).
9. **Target group do hub**: tipo `ip`, protocolo **HTTP**, porta **80**, na VPC **hub**, com os dois endereços fixos do NLB como `aws_lb_target_group_attachment`. Registrar IP fora da VPC do load balancer é suportado para faixas RFC 1918 alcançáveis por TGW — é o caso.
10. **Listener rule** com `condition { host_header { values = ["*.<var.name>.nonprod.<domínio>"] } }` e `action` de forward para a target group do hub. `priority` precisa ser único no listener — derivar de algo estável da célula, não `count.index`.
11. **Registro DNS** na subzona: `*.<var.name>.nonprod.<domínio>` como **A alias** para o ALB (`alb_dns_name` + `alb_zone_id`). Wildcard no DNS evita um registro por app, casando com o wildcard do certificado.
