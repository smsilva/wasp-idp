...
**Pilar WAF principal:** Security (SEC05 — proteção de recursos de rede; SEC06 — computação;
SEC04 — detecção).
...
- **SEC04-BP01**: sem Flow Logs, uma violação de isolamento é invisível até alguém reclamar.
...
|---|---|
| **SEC05-BP01** múltiplas camadas | conta → TGW RT → SG → NACL |
| **SEC05-BP02** todas controlam tráfego | SG stateful + NACL stateless + rotas |
| **SEC05-BP03** endpoints privados | Gateway/Interface endpoints evitam exposição pública |
| **SEC04-BP01** capturar logs | VPC Flow Logs em todos os spokes |
