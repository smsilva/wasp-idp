# 01 — Conta por Tenant (o silo levado à conta AWS)
...
Levar o silo ([`00-modelos-de-tenancy.md`](00-modelos-de-tenancy.md)) até a conta AWS é a forma
...
- **A conta não nasce na OU de destino** — nasce na Root e é movida depois. Na janela entre os
  dois passos a SCP da OU **não vale** (`../accounts/03-provisionamento-de-contas.md`).
- **Conta nova não está no portal SSO** — o único gancho é a `OrganizationAccountAccessRole` até
  um permission set ser atribuído (`../accounts/04-acesso-cross-account.md`).
...
| Exigência é residência geográfica | conta por tenant **não resolve** — é OU por geografia ([`02-ou-por-geografia.md`](02-ou-por-geografia.md)) |
...
uma segunda conta. Detalhe em `../accounts/00-estrategia-de-contas.md`, seção "Conta não tem
região", e a consequência para IAM em `../security/08-identidade-do-control-plane.md`.

O que **sim** varia por região é o consumo de CIDR — e é aí que o plano de endereçamento estoura
([`03-cidr-e-tenancy.md`](03-cidr-e-tenancy.md)).
...
→ [`02-ou-por-geografia.md`](02-ou-por-geografia.md): por que a lista de regiões de um tenant é
